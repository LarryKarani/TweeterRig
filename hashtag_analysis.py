...
def usage():
    print("usage:")
    print(f"python: {sys.argv[0]} <filename.json1>")
...
if __name__ == '__main__':
    ...
    with open(fname, 'r') as f:
        ...
        for line in f:
            ...
            hashtag_in_tweet = get_hashtags(tweet)
            n_of_hashtags = len(hashtag_in_tweet)
            hashtag_count[n_of_hashtags] +=1  

        tweets_with_hashtags = sum([count for n_of_tags, count in hashtag_count.items() if n_of_tags])
        tweets_no_hashtags = hashtag_count[0]
        tweets_total = tweets_no_hashtags + tweets_with_hashtags

        tweets_with_hashtags_percent = "%.2f" %(tweets_no_hashtags/tweets_total *100)
        tweets_no_hashtags_percent = "%.2f" % (tweets_no_hashtags/tweets_total *100)

        print("{} tweets without hashtags ({}%)".format(tweets_no_hashtags, tweets_with_hashtags_percent))
